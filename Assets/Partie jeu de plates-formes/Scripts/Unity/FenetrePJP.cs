...
enum DRAWOPTION2 {select, paint, paintover, erase};


#if UNITY_EDITOR
public class FenetrePJP : EditorWindow
{
	...
	private static DRAWOPTION2 selected;
	...
	private string tilemapsParentDirectory = "/Partie jeu de plates-formes/Sprites/";
	private string tilemapsDirectoryName = "Environnements"; // "/Tilemaps/"
	public GUIStyle textureStyle;
	public GUIStyle textureStyleAct;
	
	[MenuItem("Tools/Partie jeu de plates-formes")]
	private static void TilemapEditor()
	{
		EditorWindow.GetWindow(typeof (FenetrePJP));
	}
		...
		static void OnSceneGUI(SceneView aView)
		{
			if (isEnabled)
			{
				if(isDraw)
				{
					...
					if (selected != DRAWOPTION2.select)
					{
						...
						if ((e.type == EventType.MouseDrag || e.type == EventType.MouseDown) && e.button == 0 && activeSprite != null)
						{
							...
							else if (selected == DRAWOPTION2.paintover)
							{
								for (int i = 0; i < allgo.Length;i++)
								{
							...
							else if (selected == DRAWOPTION2.erase)
							{
								for (int i = 0; i < allgo.Length;i++)
								{
									if (Mathf.Approximately(allgo[i].transform.position.x, mouseWorldPos.x) && Mathf.Approximately(allgo[i].transform.position.y, mouseWorldPos.y) && Mathf.Approximately(allgo[i].transform.position.z, mouseWorldPos.z))
	...
	void OnGUI()
	{
		...
		if (!Directory.Exists(Application.dataPath + tilemapsParentDirectory + tilemapsDirectoryName + "/"))
		{
			// Directory.CreateDirectory(Application.dataPath + "/Tilemaps/");
			AssetDatabase.CreateFolder("Assets" + tilemapsParentDirectory, tilemapsDirectoryName);
			AssetDatabase.Refresh();
			Debug.Log("Created Tilemaps Directory");
		}
		files = Directory.GetFiles(Application.dataPath + tilemapsParentDirectory + tilemapsDirectoryName + "/", "*.png");
		options = new string[files.Length];
		EditorGUILayout.LabelField("Tile Map", GUILayout.Width(256));
		for(int i = 0; i < files.Length; i++)
		{
			options[i] = files[i].Replace(Application.dataPath + tilemapsParentDirectory + tilemapsDirectoryName + "/", "");
		}
		...
		selected = (DRAWOPTION2)EditorGUILayout.EnumPopup(selected, GUILayout.Width(236));
		GUILayout.EndHorizontal();
		...
		{
			allSprites = AssetDatabase.LoadAllAssetsAtPath("Assets" + tilemapsParentDirectory + tilemapsDirectoryName + "/" + options[index]).Select(x => x as Sprite).Where(x => x != null).ToArray();	
