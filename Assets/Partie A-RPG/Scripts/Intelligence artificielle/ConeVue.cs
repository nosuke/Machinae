...
public class ConeVue : MonoBehaviour {
	...
	void Start () {
		...
		for (int i = 0; i < précision; i++) {
			Matrix4x4 mat = Matrix4x4.TRS (Vector2.zero, Quaternion.Euler(0, 0, (débutAngle + i*pasAngle)), Vector2.one);
			directions[i] = mat * Vector2.up;
		}
